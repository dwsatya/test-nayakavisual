  ...
  useEffect(() => {
    const fetchEventFolder = async () => {
      try {
        ...
        const eventTerpilih = data.find(ev => ev.edited_id === 1);

        if (eventTerpilih && eventTerpilih.kode_folder) {
          setFolderId(eventTerpilih.kode_folder);
          console.log("Folder ditemukan dengan edited_id 1:", eventTerpilih.kode_folder);
        } else {
          console.error("Event dengan edited_id 1 tidak ditemukan");
          setLoading(false);
        }
      } catch (err) {
